...
var TAG_CASES = ["camelCase", "PascalCase", "snake_case", "kebab-case", "iKebab-case"];
...
var META_FIELDS = /* @__PURE__ */ new Map([
  ["excerpt", /* @__PURE__ */ new Set(["description", "og:description", "twitter:description"])],
  ["byline", /* @__PURE__ */ new Set(["author", "article:author", "parsely-author", "cXenseParse:author"])],
  // article:author is a think, but seems mostly for bio URLs
  ["title", /* @__PURE__ */ new Set(["title", "og:title", "parsely-title", "twitter:title", "twitter:text:title"])],
  ["siteName", /* @__PURE__ */ new Set(["og:site_name", "page.content.source", "application-name", "apple-mobile-web-app-title", "twitter:site"])],
  ["publishedTime", /* @__PURE__ */ new Set(["article:published_time", "parsely-pub-date", "datePublished", "article.published"])],
  ["modifiedTime", /* @__PURE__ */ new Set(["article:modified_time", "dateModified", "dateLastPubbed"])],
  ["link", /* @__PURE__ */ new Set(["url", "og:url", "parsely-link", "twitter:url"])],
  ["type", /* @__PURE__ */ new Set(["og:type", "parsely-type", "medium", "page.content.type"])],
  ["twitter", /* @__PURE__ */ new Set(["twitter:creator", "twitter:site"])],
  ["tags", /* @__PURE__ */ new Set(["tags", "keywords", "article:tag", "parsely-tags", "news_keywords"])],
  ["onion", /* @__PURE__ */ new Set(["onion-location"])]
  // http-equiv
]);
var SlurpPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.displayError = (err) => new import_obsidian.Notice(`Slurp Error! ${err.message}`);
  }
  async onload() {
  ...
  parseMetadataTags(elements) {
    const tags = /* @__PURE__ */ new Set();
    elements.forEach((e) => e.content.split(",").forEach((text) => {
  ...
  parseMetadata(doc) {
    const metadata = { tags: /* @__PURE__ */ new Set() };
    const tmpl = 'meta[name="{attr}"], meta[property="{attr}"], meta[itemprop="{attr}"], meta[http-equiv="{attr}"]';
    META_FIELDS.forEach((attrs, k) => {
      attrs.forEach((attr) => {
        if (k == "tags") {
          const elements = doc.querySelectorAll(tmpl.replace("{attr}", attr));
          metadata.tags = /* @__PURE__ */ new Set([...metadata.tags, ...this.parseMetadataTags(elements)]);
        } else {
  ...
  mergeMetadata(article, metadata) {
    ...
    merged.tags = /* @__PURE__ */ new Set([...article.tags, ...metadata.tags]);
  ...
  createContent(article) {
    const fmtDt = (dt) => {
      return (0, import_obsidian.moment)(dt || new Date()).format("YYYY-MM-DDTHH:mm");
    };
    const maybe = (k, v) => {
      if (!isEmpty(v) || this.settings.showEmptyProps)
        return `${k}: ${v || ""}
`;
      else
        return "";
  ...
  - ${val}`);
    ...
    const modifiedTime = article.modifiedTime ? fmtDt(article.modifiedTime) : void 0;
    const twitter = article.twitter ? `https://twitter.com/${article.twitter.substring(1)}` : void 0;
    let noteProps = "---\n";
...
var SlurpSettingsTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Show empty properties").setDesc("Should Slurp add all note properties even if they are empty?").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.showEmptyProps).onChange(async (val) => {
        this.plugin.settings.showEmptyProps = val;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Parse tags").setDesc("Use the tags and keywords discovered in slurped page metadata? WARNING: May result in a large number of new tags, prefixes are highly recommended. Some sites put entire sentences into the fields meant for comma-separated keywords. Yes, abc7news.com, I'm talking about you. ").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.parseTags).onChange(async (val) => {
        this.plugin.settings.parseTags = val;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Tag prefix").setDesc("Apply this prefix to all tags.").addText(
      (text) => text.setValue(this.plugin.settings.tagPrefix).setDisabled(!this.plugin.settings.parseTags).onChange(async (val) => {
        this.plugin.settings.tagPrefix = val;
        await this.plugin.saveSettings();
      })
    );
    const tagCaseOptions = TAG_CASES.reduce((acc, cur) => {
      acc[cur] = cur;
      return acc;
    }, {});
    new import_obsidian.Setting(containerEl).setName("Tag case").setDesc("Format multi-word tags using this style. iKebab-case will replace spaces with hyphens without changing case.").addDropdown(
      (dropdown) => dropdown.addOptions(tagCaseOptions).setValue(this.plugin.settings.tagCase).setDisabled(!this.plugin.settings.parseTags).onChange(async (val) => {
